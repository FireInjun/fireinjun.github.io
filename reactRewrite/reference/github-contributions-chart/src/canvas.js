...
function drawYear(ctx, opts = {}) {
  const {year, offsetX = 0, offsetY = 0, data, fontFace = defaultFontFace} =
      opts;
  ...
  for (let i = 1; i < 7; i += 1) {
    graphEntries.push(firstRowDates.map(dateObj => {
      ...
      return {date, info: getDateInfo(data, date)};
    }));
  }

  const count =
      new Intl.NumberFormat().format(getContributionCount(graphEntries));
  ...
  for (let y = 0; y < graphEntries.length; y += 1) {
    for (let x = 0; x < graphEntries[y].length; x += 1) {
      ...
      ctx.fillRect(
          ...
          offsetY + textHeight + (boxWidth + boxMargin) * y, 10, 10);
    }
  }
  ...
  for (let y = 0; y < graphEntries[0].length; y += 1) {
    ...
    if (monthChanged && !firstMonthIsDec) {
      ...
      ctx.fillText(
          date.format('MMM'), offsetX + (boxWidth + boxMargin) * y, offsetY);
...
function drawMetaData(ctx, opts = {}) {
  const {username, width, height, footerText, fontFace = defaultFontFace} =
      opts;
  const theme = getTheme(opts);
  ...
  ctx.fillText(
      'Less',
      width - canvasMargin - (boxWidth + boxMargin) * (themeGrades) - 55, 37);
...
const drawContributions(canvas, opts) => {
  ...
  drawMetaData(ctx, {...opts, width, height});
